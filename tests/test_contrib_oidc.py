...
def test_authorized_signup(app_with_userprofiles, example_oidc):
    ...
    with app.test_client() as c:
        ...
        assert resp.status_code == 302
        signup_url = url_for("invenio_oauthclient.signup", remote_app="oidc")
        # Depending on app form configuration, signup can complete automatically
        # or require the extra signup form.
        assert resp.location in ["/", "/account/settings/linkedaccounts/", signup_url]

        if resp.location == signup_url:
            return
